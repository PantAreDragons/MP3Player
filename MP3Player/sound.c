...
void writeDACs( void ) {
  ...
  if (dataLength >0 ){
      ...
      //check to see if
      if (dataLength < PADDING) {//psuedocode
          bufferLow = true;
      }
      //need if statement to check both left and right buffers to see if they are out
      //would need to compare via ==, but not sure what to compare.

      //have it check to see if legt and right buffers are out
      //
  ...
  if(lbuffer + PADDING + bufsize == lOutPtr) {
      //lbuffer is starting address, add to the buffer size (exact offset) add
      //padding just in case we overflow buffer
     ...
     rOutPtr = rbuffer;
       ...
       dataLength = 0;
       //set to zero in case of negative value
   }
#ifdef MEASUREMENTS
