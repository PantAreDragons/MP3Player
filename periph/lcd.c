...
// FOR TIMING DELAYS.
 //deassert rd first, then write the data,
    //start asserting CS and formally initating write, THEN finish.

    assertRD(false);         //clear bits
    writeDB(c);           //initiate bit transfer
    assertCS(true);         //we can begin write after this is true
    assertWR(true);       //pulls to low
    //write
    assertWR(true);

    assertWR(false);        //set to false because we are done
    assertCS(false);
...
#define assertCS(val)      assertPIN(val,GPIO_PIN_3)
  */
  ...
  //dont set just clear M dir pur  afsel clear

//dont need alternate function so no need for alternate function
GPIO_PORTM[GPIO_DIR] = GPIO_ALLPINS;
GPIO_PORTM[GPIO_DEN] = GPIO_ALLPINS;
//TODO: fix afsel
GPIO_PORTM[GPIO_AFSEL] &= ~GPIO_ALLPINS;
GPIO_PORTM[GPIO_PUR] = GPIO_ALLPINS;
