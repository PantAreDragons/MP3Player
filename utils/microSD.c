...
bool initMicroSD( uint16_t retries ) {
  ...
  do {
    ...
    //have a step here for the oscillator
...
bool readBlock( uint32_t blockaddr, uint8_t *buffer ) {
   ...
   uint8_t tempVal;
   uint16_t i;
// Read a single block of 512 bytes from the MicroSD card
  // Adjust the address based on the type of card
  blockaddr <<= blockScaling;

  //line 136 had response declared as 32 but ran here as uint8_t so we did
  //the same
 ...
 //rxbyteSSIO to get data after reply is good
 //tempVal = rxByteSSI3();
 while( true ){
     tempVal = rxByteSSI3();
     if (tempVal == 0xfe){
